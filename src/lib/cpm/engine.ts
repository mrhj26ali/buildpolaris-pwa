...
export function calculateCpm(tasks: CpmTask[], deps: CpmDep[]) {
  const taskMap = new Map(tasks.map(t => [t.id, { ...t }]));
  const graph = new Map<string, CpmDep[]>();
  const revGraph = new Map<string, CpmDep[]>();
  
  tasks.forEach(t => { graph.set(t.id, []); revGraph.set(t.id, []); });
  deps.forEach(d => { graph.get(d.pred)?.push(d); revGraph.get(d.succ)?.push(d); });
  ...
  if (topo.length !== tasks.length) return { tasks: [], error: 'Circular dependency detected' };
