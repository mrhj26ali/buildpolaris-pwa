// src/features/financials/__tests__/financials.test.ts
import { expect, test, describe, vi } from 'vitest';
import { getBudgetSummary, getCommitmentList, type CostCodeNode, type CommitmentNode } from '../api';
...
describe('Financial Control API Client', () => {
  test('getBudgetSummary calls correct endpoint', async () => {
    ...
    expect(bffRequest).toHaveBeenCalledWith(
      '/method/buildpolaris_bff.api.financial_control.get_budget_summary',
      expect.objectContaining({
        method: 'POST',
        body: JSON.stringify({ project: 'TEST-PROJ' }),
      })
    );
    expect(result.cost_codes).toHaveLength(1);
  ...
  test('getCommitmentList calls correct endpoint', async () => {
    const mockCommitments: CommitmentNode[] = [
      {
        name: 'COM-001',
    ...
    expect(bffRequest).toHaveBeenCalledWith(
      '/method/buildpolaris_bff.api.financial_control.get_commitment_list',
      expect.objectContaining({
        method: 'POST',
        body: JSON.stringify({ project: 'TEST-PROJ' }),
      })
