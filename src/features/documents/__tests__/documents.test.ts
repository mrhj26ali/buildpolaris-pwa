// src/features/documents/__tests__/documents.test.ts
import { expect, test, describe, vi } from 'vitest';
import { getDrawingRegister, getPublishedDrawings, type DrawingNode } from '../api';
...
describe('Document Control API Client', () => {
  test('getDrawingRegister calls correct endpoint', async () => {
    const mockDrawings: DrawingNode[] = [
      {
        name: 'DWG-001',
        sheet_number: 'A-101',
    ...
    expect(bffRequest).toHaveBeenCalledWith(
      '/method/buildpolaris_bff.api.document_control.get_drawing_register',
    ...
    expect(result[0].revision_count).toBe(2);
  });

  test('getPublishedDrawings calls correct endpoint for IFC set', async () => {
    const mockPublished: DrawingNode[] = [
      {
        name: 'DWG-001',
        sheet_number: 'A-101',
        discipline: 'Architectural',
    ...
    expect(bffRequest).toHaveBeenCalledWith(
      '/method/buildpolaris_bff.api.document_control.get_published_drawings',
      expect.objectContaining({
        method: 'POST',
        body: JSON.stringify({ project: 'TEST-PROJ' }),
      })
    ...
    expect(result).toHaveLength(1);
    expect(result[0].current_revision).toBe('REV-00001');
  });
});
