...
export function UserInviteModal({ onClose, onSuccess }: Props) {
  ...
  useEffect(() => {
    ...
    availableRoles()
      ...
      .catch(() => { if (!cancelled) setError(t('admin.invite.failed')) })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t('admin.invite.failed'))
    } finally {
      setBusy(false)
    }
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4">
      <Card className="flex max-h-[90vh] w-full max-w-lg flex-col">
        <CardHeader className="shrink-0"><CardTitle className="text-base">{t('admin.invite.title')}</CardTitle></CardHeader>
        <form onSubmit={handleSubmit} className="flex min-h-0 flex-1 flex-col">
          <CardContent className="min-h-0 flex-1 space-y-4 overflow-y-auto pb-2">
            {error && <p className="text-sm text-red-600">{error}</p>}
            <div className="space-y-2">
              <Label>{t('admin.invite.fullName')}</Label>
              <Input value={form.full_name} onChange={(e) => setForm({ ...form, full_name: e.target.value })} required />
            </div>
            <div className="space-y-2">
              <Label>{t('admin.invite.email')}</Label>
              <Input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} required />
            </div>
            <div className="space-y-2">
              <Label>{t('admin.invite.roles')}</Label>
          ...
          <div className="flex shrink-0 items-center justify-between gap-2 border-t border-surface-border px-6 py-4">
            <span className="text-xs text-gray-500">{t('admin.invite.rolesSelected', { count: selectedRoles.length })}</span>
            <div className="flex gap-2">
              <Button type="button" variant="outline" onClick={onClose}>{t('common.cancel')}</Button>
              <Button type="submit" className="bg-brand-500" disabled={busy || selectedRoles.length === 0}>
                {busy ? t('admin.invite.sending') : t('admin.invite.send')}
              </Button>
            </div>
          </div>
        </form>
