...
export function UserEditModal({ user, onClose, onSuccess }: Props) {
  ...
  useEffect(() => {
    ...
    availableRoles()
      ...
      .catch(() => { if (!cancelled) setError(t('admin.edit.failed')) })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t('admin.edit.failed'))
    } finally {
      setBusy(false)
    }
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4">
      <Card className="flex max-h-[90vh] w-full max-w-lg flex-col">
        <CardHeader className="shrink-0">
          <CardTitle className="text-base">{t('admin.edit.title', { name: user.full_name })}</CardTitle>
        </CardHeader>
          ...
          <div className="flex shrink-0 items-center justify-between gap-2 border-t border-surface-border px-6 py-4">
            ...
            <div className="flex gap-2">
              ...
              <Button type="submit" className="bg-brand-500" disabled={busy || selectedRoles.length === 0}>
                {busy ? t('admin.edit.saving') : t('admin.edit.save')}
              </Button>
            </div>
          </div>
        </form>
