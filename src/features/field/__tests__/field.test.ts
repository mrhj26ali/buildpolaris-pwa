// src/features/field/__tests__/field.test.ts
import { expect, test, describe, vi } from 'vitest';
import { getDailyLogList, getPunchList, type DailyLogNode, type PunchItemNode } from '../api';
...
describe('Field Execution API Client', () => {
  test('getDailyLogList calls correct endpoint', async () => {
    ...
    expect(bffRequest).toHaveBeenCalledWith(
      '/method/buildpolaris_bff.api.field_execution.get_daily_log_list',
  ...
  test('getPunchList calls correct endpoint', async () => {
    const mockItems: PunchItemNode[] = [
      {
        name: 'PUNCH-001',
        title: 'Fix cracked tile',
        location: 'Lobby',
        assigned_to: null,
        priority: 'High',
        status: 'Open',
    ...
    expect(bffRequest).toHaveBeenCalledWith(
      '/method/buildpolaris_bff.api.field_execution.get_punch_list',
      expect.objectContaining({
        method: 'POST',
        body: JSON.stringify({ project: 'TEST-PROJ' }),
      })
